...
State Untwister::inferState()
{
    ...
    for (uint32_t index = 0; index < (m_observedOutputs->size() - stateSize); ++index)
    {
        ...
        std::vector<uint32_t> evidenceForward((std::vector<uint32_t>::const_iterator)m_observedOutputs->begin(), first);
        std::vector<uint32_t> evidenceBackward(last + 1, (std::vector<uint32_t>::const_iterator)m_observedOutputs->end());

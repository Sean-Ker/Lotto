...
from tensorflow.keras.layers import (Dense, Lambda, Layer, Dropout, TimeDistributed, Add,
                                     Concatenate, Conv1D, Input, Embedding, Activation, SeparableConv1D)
...
class Transformer:
	def __init__(self, i_tokens, o_tokens, len_limit, d_model=256, d_inner_hid=512, n_head=4, layers=2, dropout=0.1, share_word_emb=False):
...
class QANet_Block:
	def __init__(self, dim, n_head, n_conv, kernel_size, dropout=0.1, add_pos=True):
		...
		self.self_att = MultiHeadAttention(n_head=n_head, d_model=dim, dropout=dropout, mode=1)
